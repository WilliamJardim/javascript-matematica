function exemploInterminacao(x){
    return (x*8) / ((2*x) - 4);
}
...
function calculate()
{
    ...
    */
    ...
    <tr class='table-line'>
        ...
        <td class='header-td table-column'> Limite aproximado </td>
    </tr>
    `;

    for( let i = 0 ; i < pontos.length ; i++ )
    {
        const tendendoAPonto = pontos[i];
        const limite = estimar_limite(exemploInterminacao, tendendoAPonto);

        codeResults.innerHTML += `
        <tr class='table-line'>
            <td class='table-column point-column'> ${tendendoAPonto} </td>
            <td class='table-column'> ${limite} </td>
        </tr>
        `;
    }
}